...
const ensureUtils = (methodName: string): void => {
  if (!intlTelInput.validation) {
    throw new Error(
      `intlTelInput.validation is required for ${methodName}(). See: https://intl-tel-input.com/docs/utils`,
    );
  }
};
...
export class Iti {
  ...
  #createInitPromises(options: AllOptions): Promise<void> {
    ...
    const needsUtilsScriptPromise = Boolean(loadUtils) && !intlTelInput.validation;
  ...
  #maybeBindKeydownListener(): void {
    ...
    const handleKeydownEvent = (e: KeyboardEvent): void => {
      ...
      if (intlTelInput.validation && this.#maxCoreNumberLength) {
        const coreNumber = intlTelInput.validation.getCoreNumber(
          newFullNumber,
          this.#selectedCountryData?.iso2,
        );
  ...
  #maybeBindPasteListener(): void {
    ...
    const handlePasteEvent = (e: ClipboardEvent): void => {
      ...
      if (newVal.length > 5 && intlTelInput.validation) {
        let coreNumber = intlTelInput.validation!.getCoreNumber(newVal, iso2);
        ...
        while (coreNumber.length === 0 && newVal.length > 0) {
          ...
          coreNumber = intlTelInput.validation!.getCoreNumber(newVal, iso2);
        }
  ...
  #updateValFromNumber(fullNumber: string): void {
    ...
    if (formatOnDisplay && intlTelInput.validation && this.#selectedCountryData) {
      const isRegionless = hasRegionlessDialCode(fullNumber);
      const useNational =
        (nationalMode && !isRegionless) ||
        (!number.startsWith("+") && !separateDialCode);
      const { NATIONAL, INTERNATIONAL } = intlTelInput.validation.numberFormat;
      const format = useNational ? NATIONAL : INTERNATIONAL;
      number = intlTelInput.validation.formatNumber(
        number,
        this.#selectedCountryData?.iso2,
        format,
      );
  ...
  #updateMaxLength(): void {
    const { strictMode, placeholderNumberType, allowedNumberTypes } =
      this.#options;
    if (!strictMode || !intlTelInput.validation) {
      return;
    }
    ...
    const numberType = intlTelInput.validation.numberType[placeholderNumberType];
    let exampleNumber = intlTelInput.validation.getExampleNumber(
      iso2,
      false,
      numberType,
      true,
    ...
    while (
      intlTelInput.validation.isPossibleNumber(
        exampleNumber,
        iso2,
        allowedNumberTypes,
      )
    ...
    const coreNumber = intlTelInput.validation.getCoreNumber(validNumber, iso2);
  ...
  #updatePlaceholder(): void {
    ...
    if (!intlTelInput.validation || !shouldSetPlaceholder) {
      return;
    }
    ...
    let placeholder = this.#selectedCountryData
      ? intlTelInput.validation.getExampleNumber(
          this.#selectedCountryData.iso2,
          nationalMode,
          numberType,
        )
  ...
  public getExtension(): string {
    ...
    return intlTelInput.validation!.getExtension(
      this.#getFullNumber(),
      this.#selectedCountryData?.iso2,
    );
  }
  ...
  public getNumber(format?: number): string {
    ...
    const iso2 = this.#selectedCountryData?.iso2;
    const fullNumber = this.#getFullNumber();
    const formattedNumber = intlTelInput.validation!.formatNumber(
      fullNumber,
      iso2,
      format,
    );
  ...
  public getNumberType(): number {
    ...
    return intlTelInput.validation!.getNumberType(
      this.#getFullNumber(),
      this.#selectedCountryData?.iso2,
    );
  }
  ...
  public getValidationError(): number {
    ...
    const iso2 = this.#selectedCountryData?.iso2;
    return intlTelInput.validation!.getValidationError(this.#getFullNumber(), iso2);
  }
  ...
  public isValidNumber(): boolean | null {
    ...
    const coreNumber = intlTelInput.validation!.getCoreNumber(number, iso2);
    if (coreNumber) {
  ...
  #utilsIsPossibleNumber(val: string): boolean | null {
    return intlTelInput.validation
      ? intlTelInput.validation.isPossibleNumber(
          val,
          this.#selectedCountryData?.iso2,
  ...
  #validateNumber(precise: boolean): boolean | null {
    ...
    if (hasAlphaChar) {
      ...
      const hasExtension = Boolean(
        intlTelInput.validation!.getExtension(val, selectedIso2),
      );
      if (hasExtension) {
  ...
  #utilsIsValidNumber(val: string): boolean | null {
    return intlTelInput.validation
      ? intlTelInput.validation.isValidNumber(
          val,
          this.#selectedCountryData?.iso2,
          this.#options.allowedNumberTypes,
        )
...
const attachUtils = (source: UtilsLoader): Promise<boolean> | null => {
  ...
  if (!intlTelInput.validation && !intlTelInput.startedLoadingUtilsScript) {
    let loadCall;
